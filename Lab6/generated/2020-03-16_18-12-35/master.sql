...
spool "Lab6_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_Lab6 ...
DROP ROLE ROLE_Lab6 ;
PROMPT Creating Role ROLE_Lab6 ...
CREATE ROLE ROLE_Lab6 ;

-- PROMPT Drop publishing_house user
-- drop user publishing_house cascade;
   
PROMPT Create user publishing_house
CREATE USER publishing_house identified by &&publishing_house_password DEFAULT TABLESPACE USERS TEMPORARY TABLESPACE TEMP;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM TO publishing_house;
...
GRANT ALL ON utils TO ROLE_Lab6;
grant ALL on utils to publishing_house with grant option;

set define on
prompt connecting to publishing_house
connect publishing_house/&&publishing_house_password;
set define off

DROP SEQUENCE orders_Id_order_SEQ;


PROMPT Creating Sequence orders_Id_order_SEQ ...
CREATE SEQUENCE  orders_Id_order_SEQ  
  MINVALUE 1 MAXVALUE 999999999999999999999999 INCREMENT BY 1  NOCYCLE ;

DROP SEQUENCE production_Id_prod_SEQ;


PROMPT Creating Sequence production_Id_prod_SEQ ...
CREATE SEQUENCE  production_Id_prod_SEQ  
  MINVALUE 1 MAXVALUE 999999999999999999999999 INCREMENT BY 1  NOCYCLE ;

DROP SEQUENCE client_Id_client_SEQ;


PROMPT Creating Sequence client_Id_client_SEQ ...
...
CREATE TABLE client (
  ...
  Name_client NVARCHAR2(50),
  Address_client NVARCHAR2(50),
  hid BLOB
);


PROMPT Creating Primary Key Constraint PK__client__8829B11E7B87B890 on table client ... 
ALTER TABLE client
ADD CONSTRAINT PK__client__8829B11E7B87B890 PRIMARY KEY
(
  Id_client
)
ENABLE
...
GRANT ALL ON client TO ROLE_Lab6;
...
CREATE TABLE orders (
  ...
  Date_order_start VARCHAR2(200 CHAR),
  Date_order_end VARCHAR2(200 CHAR),
  Client NUMBER(10,0),
  Production NUMBER(10,0),
  Cost_order NUMBER(10,0)
);


PROMPT Creating Primary Key Constraint PK__orders__33F95B5CE572B876 on table orders ... 
ALTER TABLE orders
ADD CONSTRAINT PK__orders__33F95B5CE572B876 PRIMARY KEY
(
  Id_order
)
ENABLE
...
GRANT ALL ON orders TO ROLE_Lab6;
DROP TABLE user_log CASCADE CONSTRAINTS;


PROMPT Creating Table user_log ...
CREATE TABLE user_log (
  u_date VARCHAR2(200 CHAR) NOT NULL,
  u_time VARCHAR2(200 CHAR) NOT NULL,
...
GRANT ALL ON user_log TO ROLE_Lab6;
...
CREATE TABLE Author (
  ...
  birthday VARCHAR2(200 CHAR),
  Phone NVARCHAR2(10)
);


PROMPT Creating Primary Key Constraint PK__Author__E26DB52434522165 on table Author ... 
ALTER TABLE Author
ADD CONSTRAINT PK__Author__E26DB52434522165 PRIMARY KEY
(
  Surname
)
ENABLE
...
GRANT ALL ON Author TO ROLE_Lab6;
DROP TABLE Type_of_prod CASCADE CONSTRAINTS;


PROMPT Creating Table Type_of_prod ...
CREATE TABLE Type_of_prod (
  id NUMBER(10,0),
  Type_prod NVARCHAR2(15) NOT NULL
);


PROMPT Creating Primary Key Constraint PK__Type_of___2884FB79BA657BEE on table Type_of_prod ... 
ALTER TABLE Type_of_prod
ADD CONSTRAINT PK__Type_of___2884FB79BA657BEE PRIMARY KEY
(
  Type_prod
)
ENABLE
...
GRANT ALL ON Type_of_prod TO ROLE_Lab6;
DROP TABLE production CASCADE CONSTRAINTS;


PROMPT Creating Table production ...
CREATE TABLE production (
  Id_prod NUMBER(10,0) NOT NULL,
  Type_prod NVARCHAR2(15),
  Title_prod NVARCHAR2(50),
  Author_prod NVARCHAR2(20)
);


PROMPT Creating Primary Key Constraint PK__producti__5FFE1E8E33E9CC30 on table production ... 
ALTER TABLE production
ADD CONSTRAINT PK__producti__5FFE1E8E33E9CC30 PRIMARY KEY
(
  Id_prod
)
ENABLE
...
GRANT ALL ON production TO ROLE_Lab6;
...
CREATE OR REPLACE FORCE VIEW order_view 
AS 
   SELECT orders.Date_order_start ,
          client.Name_client ,
          production.Title_prod ,
          production.Author_prod ,
          orders.Cost_order 
     FROM ( ( orders 
              JOIN client    ON orders.Client = client.Id_client
               ) 
            JOIN production    ON orders.Production = production.Id_prod
             ) ;


GRANT ALL ON order_view TO ROLE_Lab6;
CREATE OR REPLACE FUNCTION countOrdersOfClient
(
  v_datastarta IN VARCHAR2,
  v_dataend IN VARCHAR2
)
RETURN NUMBER
AS
   v_ret NUMBER(10,0);
...
BEGIN
   SELECT COUNT(*)  
     ...
     INTO v_ret
     FROM orders 
    WHERE  Date_order_start BETWEEN v_datastarta AND v_dataend;
...
END;
/
GRANT ALL ON countOrdersOfClient TO ROLE_Lab6;
CREATE OR REPLACE FUNCTION publishing_house.fn_diagramobjects
RETURN NUMBER
AUTHID DEFINER
AS
...
END;
/
GRANT ALL ON fn_diagramobjects TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE add_author
(
  v_surname IN NVARCHAR2,
  v_birthday IN NVARCHAR2,
...
BEGIN
   ...
   INSERT INTO Author
     ( Surname, birthday, Phone )
     VALUES ( v_surname, v_birthday, v_phone );
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON add_author TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE add_client
(
  v_name IN NVARCHAR2,
  v_adress IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
AS
...
END;
/
GRANT ALL ON add_client TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE AddEmp
(
  v_name_c IN NVARCHAR2,
  v_add IN NVARCHAR2,
  v_old_uzel IN NUMBER
)
AS
   v_nold UDT.HIERARCHYID%TYPE;
   v_nnew UDT.HIERARCHYID%TYPE;
...
BEGIN
   ...
   SELECT hid 
     ...
     INTO v_nold
     FROM client 
    WHERE  Id_client = v_old_uzel;-- Получаем код узла, который хотим переподчинить со всеми его потомками
   /*TODO:SQLDEV*/ SET TRANSACTION ISOLATION LEVEL SERIALIZABLE /*END:SQLDEV*/
   ...
   SELECT MAX(hid)  
     ...
     INTO v_nnew
     FROM client 
    WHERE  GetAncestor(1) = v_nold -- Получаем код узла нового родителя
   ;client ( Name_client , Address_client , hid ) VALUES ( @name_c , @add , @nold . GetDescendant ( @nnew , NULL ) )  --SQLDEV: NOT RECOGNIZED
   utils.commit_transaction;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON AddEmp TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE add_order
(
  v_date_start IN NVARCHAR2,
  v_date_end IN NVARCHAR2,
  v_client_order IN NUMBER,
  v_production_order IN NUMBER,
  v_cost_order IN NUMBER,
  cv_1 OUT SYS_REFCURSOR
)
AS
...
BEGIN
   ...
   INSERT INTO orders
     ( Date_order_start, Date_order_end, Client, Production, Cost_order )
     VALUES ( v_date_start, v_date_end, v_client_order, v_production_order, v_cost_order );
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON add_order TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE add_prod
(
  v_type_prod IN NVARCHAR2,
  v_title_prod IN NVARCHAR2,
  v_author_prod IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
AS
...
BEGIN
   ...
   INSERT INTO production
     ( Type_prod, Title_prod, Author_prod )
     VALUES ( v_type_prod, v_title_prod, v_author_prod );
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON add_prod TO ROLE_Lab6;
...
BEGIN
   ...
   INSERT INTO Type_of_prod
     ( id, Type_prod )
     VALUES ( v_id_type, v_type );
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON add_type TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE change_author
(
  v_surname IN NVARCHAR2,
  v_birthday IN NVARCHAR2,
  v_phone IN NVARCHAR2,
...
END;
/
GRANT ALL ON change_author TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE change_client
(
  v_name IN NVARCHAR2,
  v_adress IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
...
BEGIN
   ...
   UPDATE client
      SET Name_client = v_name,
          Address_client = v_adress
    WHERE  Name_client = v_name;
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON change_client TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE change_order
(
  v_date_start IN NVARCHAR2,
  v_date_end IN NVARCHAR2,
  v_client_order IN NUMBER,
...
BEGIN
   ...
   UPDATE orders
      SET Date_order_start = v_date_start,
          Date_order_end = v_date_end,
          Client = v_client_order,
          Production = v_production_order,
          Cost_order = v_cost_order
    WHERE  Date_order_start = v_date_start
     AND Date_order_end = v_date_end;
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON change_order TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE change_prod
----
...
BEGIN
   ...
   UPDATE production
      SET Type_prod = v_type_prod,
          Title_prod = v_title_prod,
          Author_prod = v_author_prod
    WHERE  Type_prod = v_type_prod;
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON change_prod TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE change_type
(
  v_type IN NVARCHAR2,
  v_id_type IN NUMBER,
  cv_1 OUT SYS_REFCURSOR
...
BEGIN
   ...
   UPDATE Type_of_prod
      SET Type_prod = v_type,
          id = v_id_type
    WHERE  Type_prod = v_type;
   OPEN  cv_1 FOR
      SELECT 0 
        FROM DUAL  ;
...
END;
/
GRANT ALL ON change_type TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE drop_author
(
  v_surname IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
...
END;
/
GRANT ALL ON drop_author TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE drop_client
(
  v_name IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
...
END;
/
GRANT ALL ON drop_client TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE drop_order
(
  v_date_start IN NVARCHAR2,
  v_date_end IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
...
END;
/
GRANT ALL ON drop_order TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE drop_prod
----
...
END;
/
GRANT ALL ON drop_prod TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE DROP_TYPE
(
  v_type IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
...
END;
/
GRANT ALL ON drop_type TO ROLE_Lab6;
...
END;
/
GRANT ALL ON getAllAuthors TO ROLE_Lab6;
...
END;
/
GRANT ALL ON getAllClients TO ROLE_Lab6;
...
END;
/
GRANT ALL ON getAllOrder TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE getAllProd
----
...
END;
/
GRANT ALL ON getAllProd TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE getAllTypes
(
  cv_1 OUT SYS_REFCURSOR
)
AS
...
END;
/
GRANT ALL ON getAllTypes TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE OrdersOfClient
(
  v_datastarta IN NVARCHAR2,
  v_dataend IN NVARCHAR2,
  cv_1 OUT SYS_REFCURSOR
)
AS
...
END;
/
GRANT ALL ON OrdersOfClient TO ROLE_Lab6;
CREATE OR REPLACE PROCEDURE SelectPodchRoot
(
  v_level IN NUMBER,
  cv_1 OUT SYS_REFCURSOR
)
...
BEGIN
   ...
   OPEN  cv_1 FOR
      SELECT * 
        FROM client 
       WHERE  GetLevel() = v_level ;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON SelectPodchRoot TO ROLE_Lab6;
CREATE OR REPLACE FUNCTION publishing_house.sp_creatediagram
(
...
BEGIN
   ...
   @IsDbo = IS_MEMBER ( N'db_owner' ) ; REVERT  --SQLDEV: NOT RECOGNIZED
   REVERT  --SQLDEV: NOT RECOGNIZED
   ...
   BEGIN
      SELECT 1 INTO v_temp
       ...
       WHERE EXISTS ( SELECT diagram_id 
                      FROM publishing_house.sysdiagrams 
   ...
   END;
   END IF;
   INSERT INTO publishing_house.sysdiagrams
...
END;
/
GRANT ALL ON sp_creatediagram TO ROLE_Lab6;
/* Translation Extracted DDL For Required Objects*/
CREATE TABLE publishing_house.sysdiagrams
(
...
/
CREATE SEQUENCE tt_publishing_house.sysdiag 
;
/
CREATE OR REPLACE TRIGGER 
 tt_TR_lishing_house.sysdiag    BEFORE INSERT 
   ON publishing_house.sysdiagrams
   FOR EACH ROW
   BEGIN
      SELECT tt_publishing_house.sysdiag.NEXTVAL INTO :NEW.diagram_id
        FROM DUAL;
   END;
/


CREATE OR REPLACE FUNCTION publishing_house.sp_upgraddiagrams
RETURN NUMBER
AS
...
BEGIN
   ...
   IF utils.object_id(N'dbo.sysdiagrams') IS NOT NULL THEN
    RETURN 0;
   END IF;
   DELETE FROM publishing_house.sysdiagrams;
   ...
   BEGIN
      INSERT INTO publishing_house.sysdiagrams
        ...
        ( SELECT UTILS.CONVERT_TO_VARCHAR2(dgnm.uvalue,128) ,
                 ...
                 dgdef.lvalue 
          FROM publishing_house.dtproperties dgnm
                 JOIN publishing_house.dtproperties dggd   ON dggd.property = 'DtgSchemaGUID'
                 AND dggd.objectid = dgnm.objectid
                 JOIN publishing_house.dtproperties dgdef   ON dgdef.property = 'DtgSchemaDATA'
...
END;
/
GRANT ALL ON sp_upgraddiagrams TO ROLE_Lab6;
set define on
prompt connecting to publishing_house
connect publishing_house/&&publishing_house_password;
set define off

PROMPT Creating Foreign Key Constraint FK__productio__Autho__3D5E1FD2 on table Author...
ALTER TABLE production
ADD CONSTRAINT FK__productio__Autho__3D5E1FD2 FOREIGN KEY
(
  Author_prod
)
REFERENCES Author
...
PROMPT Creating Foreign Key Constraint FK__orders__Client__787EE5A0 on table client...
ALTER TABLE orders
ADD CONSTRAINT FK__orders__Client__787EE5A0 FOREIGN KEY
(
  Client
)
REFERENCES client
...
PROMPT Creating Foreign Key Constraint FK__orders__Producti__797309D9 on table production...
ALTER TABLE orders
ADD CONSTRAINT FK__orders__Producti__797309D9 FOREIGN KEY
(
  Production
)
...
PROMPT Creating Foreign Key Constraint FK__productio__Type___3E52440B on table Type_of_prod...
ALTER TABLE production
ADD CONSTRAINT FK__productio__Type___3E52440B FOREIGN KEY
(
  Type_prod
)
REFERENCES Type_of_prod
(
  Type_prod
)
ENABLE
...
CREATE OR REPLACE TRIGGER orders_Id_order_TRG BEFORE INSERT ON orders
FOR EACH ROW
DECLARE 
v_newVal NUMBER(12) := 0;
v_incval NUMBER(12) := 0;
BEGIN
  IF INSERTING AND :new.Id_order IS NULL THEN
    SELECT  orders_Id_order_SEQ.NEXTVAL INTO v_newVal FROM DUAL;
    -- If this is the first time this table have been inserted into (sequence == 1)
    IF v_newVal = 1 THEN 
      --get the max indentity value from the table
      SELECT NVL(max(Id_order),0) INTO v_newVal FROM orders;
      v_newVal := v_newVal + 1;
      --set the sequence to that value
      LOOP
           EXIT WHEN v_incval>=v_newVal;
           SELECT orders_Id_order_SEQ.nextval INTO v_incval FROM dual;
      END LOOP;
    END IF;
  -- save this to emulate @@identity
  utils.identity_value := v_newVal; 
   -- assign the value from the sequence to emulate the identity column
   :new.Id_order := v_newVal;
  END IF;
END;

/

CREATE OR REPLACE TRIGGER production_Id_prod_TRG BEFORE INSERT ON production
FOR EACH ROW
DECLARE 
v_newVal NUMBER(12) := 0;
v_incval NUMBER(12) := 0;
BEGIN
  IF INSERTING AND :new.Id_prod IS NULL THEN
    SELECT  production_Id_prod_SEQ.NEXTVAL INTO v_newVal FROM DUAL;
    -- If this is the first time this table have been inserted into (sequence == 1)
    IF v_newVal = 1 THEN 
      --get the max indentity value from the table
      SELECT NVL(max(Id_prod),0) INTO v_newVal FROM production;
      v_newVal := v_newVal + 1;
      --set the sequence to that value
      LOOP
           EXIT WHEN v_incval>=v_newVal;
           SELECT production_Id_prod_SEQ.nextval INTO v_incval FROM dual;
      END LOOP;
    END IF;
  -- save this to emulate @@identity
  utils.identity_value := v_newVal; 
   -- assign the value from the sequence to emulate the identity column
   :new.Id_prod := v_newVal;
  END IF;
END;

/

CREATE OR REPLACE TRIGGER client_Id_client_TRG BEFORE INSERT ON client
FOR EACH ROW
DECLARE 
v_newVal NUMBER(12) := 0;
v_incval NUMBER(12) := 0;
BEGIN
  IF INSERTING AND :new.Id_client IS NULL THEN
    SELECT  client_Id_client_SEQ.NEXTVAL INTO v_newVal FROM DUAL;
    -- If this is the first time this table have been inserted into (sequence == 1)
    IF v_newVal = 1 THEN 
      --get the max indentity value from the table
      SELECT NVL(max(Id_client),0) INTO v_newVal FROM client;
      v_newVal := v_newVal + 1;
      --set the sequence to that value
      LOOP
           EXIT WHEN v_incval>=v_newVal;
           SELECT client_Id_client_SEQ.nextval INTO v_incval FROM dual;
      END LOOP;
    END IF;
  -- save this to emulate @@identity
  utils.identity_value := v_newVal; 
   -- assign the value from the sequence to emulate the identity column
   :new.Id_client := v_newVal;
  END IF;
END;
...
GRANT ALL ON clobtoblob_sqldeveloper TO ROLE_Lab6;
